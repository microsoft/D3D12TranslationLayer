...
#ifndef RESIDENCY_CHECK
#define RESIDENCY_CHECK(x) \
	if((x) == false) { DebugBreak(); } __analysis_assume(x)
#endif

#ifndef RESIDENCY_CHECK_RESULT
#define RESIDENCY_CHECK_RESULT(x) \
	if(FAILED(x)) { DebugBreak(); }
#endif
#else
#ifndef RESIDENCY_CHECK
#define RESIDENCY_CHECK(x) __analysis_assume(x)
#endif
#ifndef RESIDENCY_CHECK_RESULT
...
#define MAX_NUM_CONCURRENT_CMD_LISTS 8
	...
	namespace Internal
	{
		...
		public:
				...
				pSyncManager(pSyncManagerIn)
			{
				...
				[[maybe_unused]] BOOL LuidSuccess = AllocateLocallyUniqueId(&ResidencyManagerUniqueID);
			...
			HRESULT Initialize(ID3D12Device* ParentDevice, UINT DeviceNodeIndex, IDXCoreAdapter* ParentAdapterDXCore, IDXGIAdapter3* ParentAdapterDXGI, UINT32 MaxLatency)
			{
				Device = ParentDevice;
			...
			HRESULT ExecuteCommandLists(ID3D12CommandQueue* Queue, ID3D12CommandList** CommandLists, ResidencySet** ResidencySets, UINT32 Count)
			{
				...
				HRESULT hr = GetMasterSet(ResidencySets, Count, &pMasterSet);
				if (FAILED(hr))
				{
					return hr;
				}
				if (pMasterSet == nullptr)
				{
					...
					const HRESULT LowerHR = ExecuteCommandLists(Queue, CommandLists, ResidencySets, Half);
					const HRESULT UpperHR = ExecuteCommandLists(Queue, &CommandLists[Half], &ResidencySets[Half], Count - Half);

					return (LowerHR == S_OK && UpperHR == S_OK) ? S_OK : E_FAIL;
				}
				return ExecuteMasterSet(Queue, pMasterSet, [Queue, CommandLists, Count]()
				{
			...
			HRESULT SubmitCommandQueueCommand(ID3D12CommandQueue* Queue, ResidencySet* Set, TFunc&& func)
			{
				ResidencySet* pMasterSet = nullptr;
		...
		private:
			...
			HRESULT GetMasterSet(ResidencySet** ResidencySets, UINT32 Count, ResidencySet** ppResidencySet)
			{
				HRESULT hr = S_OK;
			...
			void GetCurrentBudget(DXCoreAdapterMemoryBudget* InfoOut, DXCoreSegmentGroup Segment)
			{
                if (AdapterDXCore)
                {
                    ...
                    RESIDENCY_CHECK_RESULT(AdapterDXCore->QueryState(DXCoreAdapterState::AdapterMemoryBudget, &InputParams, InfoOut));
                }
                else
                {
                    DXGI_MEMORY_SEGMENT_GROUP SegmentDXGI = (DXGI_MEMORY_SEGMENT_GROUP)Segment;
                    DXGI_QUERY_VIDEO_MEMORY_INFO DXGIOut = {};
                    RESIDENCY_CHECK_RESULT(AdapterDXGI->QueryVideoMemoryInfo(NodeIndex, SegmentDXGI, &DXGIOut));
			...
			UINT NodeIndex;
			IDXCoreAdapter* AdapterDXCore;
            IDXGIAdapter3* AdapterDXGI;
	...
	public:
		...
		// NOTE: DeviceNodeIndex is an index not a mask. The majority of D3D12 uses bit masks to identify a GPU node whereas DXGI uses 0 based indices.
		FORCEINLINE HRESULT Initialize(ID3D12Device* ParentDevice, UINT DeviceNodeIndex, IDXCoreAdapter* ParentAdapterDXCore, IDXGIAdapter3* ParentAdapterDXGI, UINT32 MaxLatency)
		{
			return Manager.Initialize(ParentDevice, DeviceNodeIndex, ParentAdapterDXCore, ParentAdapterDXGI, MaxLatency);
		}
